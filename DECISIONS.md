...
**Substituída pela [decisão #28](#28-refresh-token-opaco-em-cookie-httponly-substitui-o-jwt-de-7-dias).**
...
## 28. Refresh token opaco em cookie httpOnly, substitui o JWT de 7 dias

**Contexto.** A decisão #13 (JWT simples de 7 dias, sem refresh) já registrava explicitamente que revogação/rotação seria candidato a uma decisão futura. Chegou a hora — usuário pediu a abordagem mais usada no mercado (a que mais aparece em processo seletivo), não a mais simples de implementar.

**Decisão.** `accessToken` (JWT) cai de 7 dias para 15 minutos de TTL. Um `refreshToken` novo cobre a renovação: não é JWT, é um valor aleatório opaco (`randomBytes(40)`), guardado no Postgres (model `RefreshToken`) só como hash sha256 — nunca em texto puro, mesma filosofia do `ipHash` (decisão #7). TTL de 30 dias. Entregue ao browser via cookie `httpOnly` (`path: /api/auth`, `secure`/`sameSite` condicionais a `NODE_ENV`), nunca em `localStorage` nem no corpo da resposta JSON — só o `accessToken` volta no JSON, como antes. `POST /api/auth/refresh` rotaciona a cada uso (revoga o token apresentado, emite um par novo); se um token já revogado for reapresentado (reuso — sinal de token roubado), revoga *todas* as sessões daquele usuário, não só recusa a chamada. `POST /api/auth/logout` revoga o token e limpa o cookie. Isso exigiu apertar o CORS (`cors({ origin: FRONTEND_URL, credentials: true })` em vez de `cors()` aberto) — cookie cross-site com `credentials: true` não funciona com origin `*` — resolvendo a pendência já registrada nas decisões #25/#27.

**Justificativa.** Cookie `httpOnly` é a prática padrão do mercado para refresh token porque JavaScript não consegue lê-lo — um XSS não rouba o refresh token, só o `accessToken` (que já teria menor janela de exposição por durar só 15 minutos). Guardar como hash em vez do valor bruto segue o mesmo raciocínio de nunca persistir segredo em texto puro que já valia pro `ipHash`. Rotação + detecção de reuso é o padrão recomendado (ex.: OAuth2 RFC 6749/Auth0/Okta) para conseguir invalidar uma sessão comprometida sem esperar o TTL de 30 dias expirar.

**Trade-offs.** Bem mais peças móveis que a decisão #13 (uma tabela nova, cookie, rotação, mutex de refresh no frontend pra não disparar múltiplas chamadas de refresh em paralelo) — aceito porque o objetivo explícito é demonstrar o padrão "profissional" num portfólio, não a solução mais simples possível. `FRONTEND_URL` vira uma env var obrigatória (fail-fast, mesmo padrão do `JWT_SECRET`) tanto local quanto no Render — como o frontend ainda não tem deploy, o valor em produção (`render.yaml`) fica com `sync: false`, exigindo preenchimento manual no painel do Render quando o frontend for deployado (documentado no README).

## 29. GeoIP offline (`geoip-country`) em vez de API externa ou MaxMind oficial

**Contexto.** O campo `Click.pais` existia desde a feature de redirect mas nunca foi preenchido (ver decisão original do redirect). Havia três caminhos: (1) uma base offline embutida no processo, (2) uma API HTTP externa de GeoIP chamada a cada clique, ou (3) a biblioteca oficial da MaxMind com conta e license key próprios.

**Decisão.** Pacote `geoip-country` (fork do `geoip-lite` restrito a granularidade de país, dados MaxMind GeoLite2), lookup síncrono em memória via `detectCountry` (`src/utils/geo.ts`), chamado em `clickService.register` só quando o clique tem IP (`input.ip`) — mesmo IP que já vira `ipHash` na mesma chamada, nunca persistido em texto puro. IP local/privado (dev local, `127.0.0.1`, `192.168.x`) ou ausente simplesmente não resolve, e `pais` fica `undefined`, igual já acontecia antes desta feature.

**Bug pré-existente encontrado e corrigido durante a implementação.** `req.ip` do Express sem `app.set("trust proxy", 1)` sempre retorna o IP do socket de conexão, ignorando `X-Forwarded-For` — atrás do load balancer do Render (ou qualquer proxy reverso), isso faz `req.ip` virar o IP interno do proxy, não o do visitante. Isso não é specific a GeoIP: já afetava o `ipHash` desde a feature de redirect original, silenciosamente (o hash sempre existia, só que do IP errado em produção). Corrigido com uma linha em `app.ts` (`app.set("trust proxy", 1)`, um único hop confiável — o load balancer do Render). Validado localmente forjando o header `X-Forwarded-For` numa chamada real ao redirect e conferindo `ipHash`/`pais` no Postgres antes/depois da correção.

**Justificativa.** ~8MB de dados embutidos (`.unpackedSize` do pacote), muito menor que o `geoip-lite`/`fast-geoip` (115–164MB) e sem custo de build/runtime perceptível — importante no plano free do Render. Lookup 100% local e síncrono: nenhuma chamada de rede em produção, então nenhum novo ponto de falha no fluxo best-effort de registro de clique, e o IP do visitante nunca sai do próprio processo — mantém a mesma postura de privacidade da decisão #7 (nunca persistir IP bruto), que uma API externa de terceiro quebraria a cada clique. A alternativa oficial da MaxMind (`@maxmind/geoip2-node`) exigiria o usuário criar conta e gerenciar uma license key, mais um processo de atualização periódica do arquivo de banco — desproporcional para um campo de enriquecimento de analytics num projeto deste porte.

**Trade-offs.** O `geoip-country` está marcado pelo próprio autor como sem mais manutenção de *código* (recomenda o sucessor `ip-location-api`, que baixa a base sob demanda em vez de embutir), mas continua publicando releases automáticas com dados atualizados do MaxMind GeoLite2 (confirmado: última publicação um dia antes desta decisão). Para granularidade de país (ao contrário de cidade/coordenadas), fronteiras mudam raramente, então o risco prático de dado desatualizado é baixo. Uso do MaxMind GeoLite2 exige atribuição (incluída no README): "This product includes GeoLite2 data created by MaxMind, available from https://www.maxmind.com".

## 30. Frontend também no Render (Static Site), em vez de Vercel/Netlify

**Contexto.** Com backend, auth completa, refresh token e GeoIP prontos, faltava só publicar o dashboard React. As opções óbvias eram Vercel/Netlify (mais associadas a SPA React especificamente) ou reaproveitar o Render, que já hospeda a API e o Postgres.

**Decisão.** Um segundo serviço no mesmo `render.yaml` (`tracer-frontend`, `runtime: static`, `rootDir: frontend`, `buildCommand: npm ci && npm run build`, `staticPublishPath: dist`), com rewrite de SPA (`/* → /index.html`) e `VITE_API_URL` fixo apontando pra API de produção já existente. Perguntado ao usuário explicitamente (Vercel vs. Render Static Site); escolheu Render.

**Justificativa.** Zero conta nova pra criar/autorizar — a mesma conta e o mesmo Blueprint que já hospedam `tracer-api`/`tracer-db` passam a hospedar o frontend também, um único painel pra tudo. Site estático no Render não tem custo/hibernação do plano free do Web Service (só o backend hiberna por inatividade). Mantém a infra 100% como código no mesmo arquivo, sem duplicar a fonte de verdade em duas plataformas diferentes.

**Trade-offs.** Vercel/Netlify têm integração mais polida especificamente pra frameworks frontend (preview automático por PR, edge network mais robusta) — abriu mão disso pela simplicidade operacional de um único provedor. `FRONTEND_URL` do `tracer-api` (decisão #28) só pode ser preenchida depois que o `tracer-frontend` existir e tiver uma URL real — os primeiros deploys do backend depois da decisão #28 ficaram presos (o processo não sobe sem essa env var, o Render mantém a versão anterior no ar em vez de derrubar o serviço) até essa etapa manual ser concluída no painel.
